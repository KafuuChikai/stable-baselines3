...
class ActorCriticSharePolicy(BasePolicy):
    """
    Policy class for actor-critic algorithms (has both policy and value prediction).
    critic use the different features extractor with the actor.

    :param observation_space: Observation space for the actor
    :param share_observation_space: Observation space for the critic
    :param action_space: Action space
    :param lr_schedule: Learning rate schedule (could be constant)
    :param net_arch: The specification of the policy and value networks.
    :param activation_fn: Activation function
    ...
    def __init__(
        ...
        self.share_features_dim = self.vf_features_extractor.features_dim
    ...
    def make_features_extractor(self, observation_space: spaces.Space) -> BaseFeaturesExtractor:
        """Helper method to create a features extractor."""
    ...
    def _get_constructor_parameters(self) -> Dict[str, Any]:
        ...
        data.update(
            dict(
                net_arch=self.net_arch,
                activation_fn=self.activation_fn,
                value_activation_fn=self.value_activation_fn,
                use_sde=self.use_sde,
                log_std_init=self.log_std_init,
                squash_output=default_none_kwargs["squash_output"],
                full_std=default_none_kwargs["full_std"],
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        #       net_arch here is an empty list and mlp_extractor does not
        #       really contain any layers (acts like an identity module).
        self.mlp_extractor = MlpShareExtractor(
            self.features_dim,
            self.share_features_dim,
            net_arch=self.net_arch,
            activation_fn=self.activation_fn,
            value_activation_fn=self.value_activation_fn,
            device=self.device,
    ...
    def forward(self, obs: th.Tensor, share_obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        Forward pass in all the networks (actor and critic)
        ...
        features = self.extract_features(obs, share_obs)
        pi_features, vf_features = features
        latent_pi = self.mlp_extractor.forward_actor(pi_features)
        latent_vf = self.mlp_extractor.forward_critic(vf_features)
        # Evaluate the values for the given observations
    ...
    def extract_features(  # type: ignore[override]
        self, obs: PyTorchObs, share_obs: PyTorchObs, features_extractor: Optional[BaseFeaturesExtractor] = None
    ) -> Union[th.Tensor, Tuple[th.Tensor, th.Tensor]]:
        """
        Preprocess the observation if needed and extract features.
        ...
        # print("obs", obs)
        # print("share_obs", share_obs)
        pi_features = self.extract_features_single(obs, self.pi_features_extractor)
        vf_features = self.extract_features_single(share_obs, self.vf_features_extractor, share_obs=True)
    ...
    def evaluate_actions(self, obs: PyTorchObs, share_obs: PyTorchObs, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        """
        Evaluate actions according to the current policy,
        given the observations.
    ...
    def get_distribution(self, obs: PyTorchObs) -> Distribution:
        ...
        :return: the action distribution.
        """
        features = self.extract_features_single(obs, self.pi_features_extractor)
        latent_pi = self.mlp_extractor.forward_actor(features)
        return self._get_action_dist_from_latent(latent_pi)

    def predict_values(self, share_obs: PyTorchObs) -> th.Tensor:
        """
        Get the estimated values according to the current policy given the observations.
        ...
        :return: the estimated values.
        """
        share_features = self.extract_features_single(share_obs, self.vf_features_extractor, share_obs=True)
        latent_vf = self.mlp_extractor.forward_critic(share_features)
        return self.value_net(latent_vf)

    def obs_to_tensor(self, observation: Union[np.ndarray, Dict[str, np.ndarray]], share_obs: bool = False) -> Tuple[PyTorchObs, bool]:
        """
        Convert an input observation to a PyTorch tensor that can be fed to a model.
        Includes sugar-coating to handle different observations (e.g. normalizing images).
        ...
        vectorized_env = False
        if share_obs:
            observation_space = self.share_observation_space
        else:
            observation_space = self.observation_space
        if isinstance(observation, dict):
            assert isinstance(
    ...
    def extract_features_single(self, obs: PyTorchObs, features_extractor: BaseFeaturesExtractor, share_obs: bool = False) -> th.Tensor:
        """
        Preprocess the observation if needed and extract features.
        ...
        """
        if share_obs:
            preprocessed_obs = preprocess_obs(obs, self.share_observation_space, normalize_images=self.normalize_images)
        else:
            preprocessed_obs = preprocess_obs(obs, self.observation_space, normalize_images=self.normalize_images)
        return features_extractor(preprocessed_obs)
